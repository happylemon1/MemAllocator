...
void* mm_malloc(size_t size) {
  ...
  block_info* ptr_allocated_block = NULL;
  ...
  // Check if there's a free block of the required size
  if (search_free_list(req_size) == NULL) {
    // If no free block of required block size, resize heap
    request_more_space(req_size);
  }

  // Call search_free_list to get pointer to valid free block that is big enough
  ptr_free_block = search_free_list(req_size);
  ptr_allocated_block = ptr_free_block;

  // Remove free block 
  remove_free_block(ptr_free_block);
  // Calculate total size of free block using macros
  block_size = SIZE(ptr_free_block->size_and_tags);



  // If there is space remaining for another free block, we must split
  if (block_size - req_size >= MIN_BLOCK_SIZE) {
    // Extract preceding_block_used_tag from free block
    preceding_block_use_tag = ((ptr_free_block->size_and_tags) & TAG_PRECEDING_USED);
    // Mark the current section as allocated by combining the allocated block size, 
    // the preceding block used tag we just extracted and marking this block as being used (allocated)
    ptr_allocated_block->size_and_tags = req_size | preceding_block_use_tag | TAG_USED;

    // Update header
    size_t* header = (size_t*) UNSCALED_POINTER_ADD(ptr_free_block, req_size);
    *header = (block_size - req_size) | TAG_PRECEDING_USED;

    // Update footer
    size_t* footer = (size_t*) UNSCALED_POINTER_ADD(ptr_free_block, block_size - WORD_SIZE);
    *footer = (block_size - req_size) | TAG_PRECEDING_USED;

    // Insert split off free block into the free list
    ptr_free_block = (block_info*) UNSCALED_POINTER_ADD(ptr_free_block, req_size);
    insert_free_block(ptr_free_block);
  } else {
    // Get next block information
    ptr_free_block = (block_info*) UNSCALED_POINTER_ADD(ptr_free_block, block_size); 
    // Next block should have tag_preceding_used information of the previous block
    ptr_free_block->size_and_tags = ptr_free_block->size_and_tags | TAG_PRECEDING_USED;
    // Set current block's boundary tags to used (allocated)
    ptr_allocated_block->size_and_tags = ptr_allocated_block->size_and_tags | TAG_USED;
  }

  // Return pointer to payload
  void* payload = (void*) UNSCALED_POINTER_ADD(ptr_allocated_block, WORD_SIZE);
  return payload;
}
...
void mm_free(void* ptr) {
  ...
  // Get pointer to start of block (header)
  block_to_free = (block_info*) UNSCALED_POINTER_SUB(ptr, WORD_SIZE);

  // Use pointer and SIZE macro to get the block's size
  payload_size = SIZE(block_to_free->size_and_tags);

  // Use block's size to get to the following block
  following_block = (block_info*) UNSCALED_POINTER_ADD(block_to_free, payload_size);
  
  // Update following block's preceding used boundary tag to indicate the previous block is not used (free)
  following_block->size_and_tags = following_block->size_and_tags ^ TAG_PRECEDING_USED; 

  // Update current (free) block's used tag to indicate it is not used (free)
  block_to_free->size_and_tags = block_to_free->size_and_tags ^ TAG_USED; 

  // Update the footer with the same size and tags information as the header
  footer = (size_t*) UNSCALED_POINTER_ADD(block_to_free, payload_size - WORD_SIZE);
